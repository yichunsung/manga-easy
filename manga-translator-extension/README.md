...
- 點擊擴充功能圖示後顯示操作選單
- 從 Popup 按鈕展開 Chrome 原生側邊欄
- 使用 `chrome.storage.local` 保存最近 100 筆原文與翻譯紀錄
- 可在 Popup 設定 OpenAI API Key 與模型
- 從選單啟動框選翻譯
- 一鍵清除目前頁面的所有翻譯結果
- 可啟用右下角常駐框選按鈕
- 擷取目前可見分頁
...
├── popup.html      # 工具列圖示的操作選單
├── popup.js        # 選單操作與目前分頁訊息
├── popup.css       # 選單樣式
├── sidepanel.html  # Chrome 側邊欄頁面
├── sidepanel.js    # 側邊欄互動
├── sidepanel.css   # 側邊欄樣式
...
1. 啟動 `manga-ocr-service`。Extension 不接受未經 MangaOCR 的後端結果。
...
3. 點擊「框選翻譯」。
4. 按住滑鼠左鍵框選日文文字區域。
5. 等待右上角的「辨識與翻譯中」提示消失。
6. 在選取區域旁查看翻譯，點擊 `×` 可關閉單一結果。

要一次關閉目前頁面的所有翻譯面板，重新打開選單並點擊
「一鍵清除所有翻譯結果」。

選擇「啟用常駐框選按鈕」後，網頁右下角會顯示「框選翻譯」小按鈕，
之後可直接點擊它開始框選。此設定會保留，並可從 Popup 選單關閉。

點擊 Popup 內的「展開側邊欄」，Chrome 會在目前視窗右側開啟 Side Panel。
...
Extension 依序顯示 `ocrText`、`romanizedText` 與 `translatedText`。

模型選單包含 GPT-5.5、GPT-5.4 系列、GPT-5.2、GPT-5.1、GPT-5、
GPT-4.1 系列、GPT-4o 系列、GPT-4 與 GPT-3.5 Turbo。無論選擇哪個模型，
後端一定先使用 MangaOCR 辨識圖片，再將純文字送給模型。Extension 也會驗證
回應中的 `ocrEngine`，拒絕任何未標示為 `manga-ocr` 的結果。
